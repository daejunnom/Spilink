...
test('TTR complete results, TTRX roundtrip and saved replay import',async({page},info)=>{
  ...
  const original=JSON.parse(await readFile(path,'utf8'));expect(original.replay.results.stats.clears).toBeTruthy();expect(original.replay.results.stats.zenith).toBeTruthy();expect(original.replay.results.stats.finaltime).toBeGreaterThan(0);expect(original.spilink.version).toBe(3);
