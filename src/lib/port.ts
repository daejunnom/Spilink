...
export type ClearEvent = { lines: number; garbageCleared: number; pc: boolean; hard: boolean; lockout?: boolean; mino: Mino; spin: 'normal' | 'mini' | 'none' };
export interface EnginePort {
  ...
  kickTable: { spawn_rotation: Partial<Record<Mino, number>> };
  ...
  spilinkHooks?: { canTick(): boolean; blockout?(): void; clear(event: ClearEvent): unknown; input?(event: KeyFrame): void; held?(): void; score?(points: number): void; next?(): Mino };
  ...
  gameOptions: { spinBonuses: string; clutch: boolean };
