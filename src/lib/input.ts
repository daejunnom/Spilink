...
export class Keyboard {
  ...
  down=(e:KeyboardEvent):void=>{
    ...
    if(!this.clock().active)return;e.preventDefault();this.pressed.set(e.code,key);this.queued.push(this.stamp('keydown',key,e.timeStamp));
  };
  up=(e:KeyboardEvent):void=>{this.physical.delete(e.code);const key=this.pressed.get(e.code);if(!key)return;this.pressed.delete(e.code);e.preventDefault();this.queued.push(this.stamp('keyup',key,e.timeStamp));};
  hoist():void{const {frame,active}=this.clock();if(!active)return;for(const code of this.physical){const entry=Object.entries(this.bindings).find(([,v])=>v===code);if(entry&& !['pause','retry','undo','redo'].includes(entry[0])){const key=entry[0] as GameKey;this.pressed.set(code,key);this.queued.push({frame,type:'keydown',data:{key,subframe:0,hoisted:true}});}}}
  release():void{const {frame}=this.clock();this.queued=[];for(const key of this.pressed.values())this.queued.push({frame,type:'keyup',data:{key,subframe:0}});this.pressed.clear();this.physical.clear();}
