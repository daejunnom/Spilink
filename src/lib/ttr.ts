...
export function makeReplay(session:Session){
  ...
  return {version:1,id:null,gamemode:'custom',ts:new Date().toISOString(),users:[{id:null,username:c.playerName,avatar_revision:0,banner_revision:0,flags:0,country:null}],
    ...
    spilink:{version:REPLAY_VERSION,profile:REPLAY_PROFILE,ruleRevision:RULE_REVISION,expectedEnd:source.expectedEnd,ticks:session.ticks,settings:structuredClone(c),changes:source.changes??[],attacks:source.attacks,first400:structuredClone(session.measure.checkpoint),usedUndo:session.usedUndo,status:session.status,summary:s,officialPlayback:false,
